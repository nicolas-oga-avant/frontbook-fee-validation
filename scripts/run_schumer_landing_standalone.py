"""Zero-tool-call capture of the schumer_box_landing surface - reuses ROADMAP 2.1's CDP port.
...
walk, then calls capture_surface(code, "schumer_landing") - no application needed, this surface
is addressed purely by the code's strategy uuid, like schumer_box_basic.
...
def run(code, headless=None, port=None, out_root=None, settle=8, landing_base=None):
    ...
    with Browser(port=port, headless=headless) as browser:
        ...
        record = ns["capture_surface"](code, "schumer_landing", settle=settle, root=out_root)
    return record
...
def main():
    ...
    record = run(args.code, headless=args.headless, port=args.port, out_root=args.out_root,
                 landing_base=args.landing_base)
    print("SCHUMER_LANDING_RESULT_JSON_START")
    print(json.dumps(record, indent=2, default=str))
    print("SCHUMER_LANDING_RESULT_JSON_END")
